...
推送 main 后 GitHub Actions 自动发布到 **https://studyguy.github.io/guandan-master-glm/**（仓库 Settings → Pages → Source 选 GitHub Actions）。仓库为私有需 GitHub Pro；公开仓库免费。
...
| 学习辅助 | 内置新手教程（5 章节）、记牌器（含队友亮牌，可折叠，窄屏默认收起）、报牌红点（≤10 张）、接风提示气泡、**纵向智能理牌三模式**（智能理牌/花色理牌/按大小，顶栏 🔀 循环切换）、**拖动选牌**（按住拖动批量选牌，点选单张，双击快出）、跨场战绩统计（localStorage） |
...
| 全屏与横屏 | 启动门一键进入全屏 + 锁定横屏；顶栏 ⛶ 随时切换；PWA「添加到主屏幕」后无浏览器界面原生运行（离线可玩） |
